    ...
    def from_dict(cls, data: Dict) -> "Company":
        ...
        if "evaluations" in data and data["evaluations"]:
            ...
            for eval_data in evaluations:
                if "skills_match" in eval_data and isinstance(
                    eval_data["skills_match"], list
                ):
                    eval_data["skills_match"] = eval_data["skills_match"]
                if "evaluated_at" in eval_data:
                    eval_data["evaluated_at"] = datetime.fromisoformat(
                        eval_data["evaluated_at"]
                    )
            data["evaluations"] = [
                CompanyEvaluation(**eval_data) for eval_data in evaluations
            ]
        else:
            data["evaluations"] = []
