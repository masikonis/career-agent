...
class CompanyVectorStore(VectorStore):
    ...
    def __init__(self, namespace: str = "prod"):
        ...
        logger.info(
            f"Connected to Pinecone index: {index_name} with namespace: {namespace}"
        )

    async def add_item(
        self, item_id: str, content: str, metadata: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Adding item {item_id} with metadata: {metadata} to namespace {self.namespace}"
            )

            self.index.upsert(
                vectors=[(item_id, vector, metadata)], namespace=self.namespace
            )
            logger.info(
                f"Successfully added item {item_id} to namespace {self.namespace}"
            )
        except Exception as e:
            logger.error(
                f"Error adding item {item_id} in namespace {self.namespace}: {e}"
            )
            raise
    ...
    async def get_item(self, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"Fetch result for {item_id} in namespace {self.namespace}: {result}"
            )

            if result and result["vectors"]:
                vector_data = result["vectors"].get(item_id)
                if vector_data and vector_data.metadata:
                    return {"id": item_id, **vector_data.metadata}
            logger.warning(
                f"No data found for item {item_id} in namespace {self.namespace}"
            )
            return None
        except Exception as e:
            logger.error(
                f"Error getting item {item_id} from namespace {self.namespace}: {e}"
            )
            raise
    ...
    async def delete_item(self, item_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully deleted item {item_id} from namespace {self.namespace}"
            )
            return True
        except Exception as e:
            logger.error(
                f"Error deleting item {item_id} from namespace {self.namespace}: {e}"
            )
            return False
    ...
    async def add_company(self, company: Company) -> None:
        ...
        try:
            ...
            upsert_response = self.index.upsert(
                vectors=[(company.id, vector, metadata)], namespace=self.namespace
            )
            ...
            for i in range(max_retries):
                result = self.index.fetch(ids=[company.id], namespace=self.namespace)
                logger.debug(f"Verification attempt {i+1}, result: {result}")

                if result and result["vectors"] and company.id in result["vectors"]:
                    logger.info(
                        f"Successfully verified company {company.id} in namespace {self.namespace}"
                    )
                    return
            ...
            raise Exception(
                f"Verification failed for company {company.id} in namespace {self.namespace} after {max_retries} attempts"
            )

        except Exception as e:
            logger.error(
                f"Error adding company {company.id} to namespace {self.namespace}: {e}"
            )
            raise
    ...
    async def get_company(self, company_id: str) -> Optional[Company]:
        ...
        try:
            logger.debug(
                f"Fetching company {company_id} from namespace {self.namespace}"
            )
            result = self.index.fetch(ids=[company_id], namespace=self.namespace)
            logger.debug(f"Fetch result: {result}")

            if result and result["vectors"]:
                vector_data = result["vectors"].get(company_id)
                if vector_data and vector_data.metadata:
                    company_data = {"id": company_id, **vector_data.metadata}
                    logger.debug(f"Reconstructed company data: {company_data}")
                    return Company.from_dict(company_data)

            logger.warning(
                f"No data found for company {company_id} in namespace {self.namespace}"
            )
            return None
        except Exception as e:
            logger.error(
                f"Error getting company {company_id} from namespace {self.namespace}: {e}"
            )
            return None
            ...
            for i in range(max_retries):
                ...
                if result and result["vectors"] and company.id in result["vectors"]:
                    ...
                    if vector_data.metadata.get("description") == company.description:
                        logger.info(
                            f"Successfully verified company update {company.id} in namespace {self.namespace}"
                        )
                        return True

                if i < max_retries - 1:
                    logger.debug(
                        f"Verification attempt {i+1} failed, waiting before retry..."
                    )
                    await asyncio.sleep(2)

            raise Exception(
                f"Update verification failed for company {company.id} in namespace {self.namespace} after {max_retries} attempts"
            )

        except Exception as e:
            logger.error(
                f"Error updating company {company.id} in namespace {self.namespace}: {e}"
            )
            return False

    async def find_similar(
        self, content: str, n_results: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for match in results.matches:
                company_data = {"id": match.id, "score": match.score, **match.metadata}
                similar_companies.append(company_data)

            return similar_companies
        except Exception as e:
            logger.error(
                f"Error finding similar companies in namespace {self.namespace}: {e}"
            )
            raise
