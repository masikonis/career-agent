...
class PineconeCompanyIndex(CompanySearchIndex):
    ...
    def __init__(self, namespace: str = "prod"):
        try:
            ...
            logger.info(f"Connected to Pinecone index: {self.index_name} namespace: {namespace}")
    ...
    async def add_to_index(self, entity_id: EntityID, company: Company, metadata: Metadata) -> bool:
        ...
        try:
            ...
            self.pinecone_index.upsert(
                vectors=[(str(entity_id), vector, metadata)],
    ...
    async def search_with_filters(
        ...
        try:
            ...
            if filters.industries:
                filter_conditions["industry"] = {"$in": [i.value for i in filters.industries]}
            if filters.stages:
